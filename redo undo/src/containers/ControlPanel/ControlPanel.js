...
const BorderWidth = ({ currBorderWidth, changeCurrBorderWidth, getFinalBorderWidth, getBorderWidthBeforeMove }) => {
  return (
    <div className="Control">
      <h3>Border width:</h3>
      <div style={{ display: "flex", alignItems: "center" }}>
        <input
          ...
          tabIndex="-1"
          style={{ width: 200 }}
          onMouseDown = {(e) => getBorderWidthBeforeMove(e.target.value)}
          onChange={(e) => changeCurrBorderWidth(e.target.value)}
          onMouseUp={(e) => getFinalBorderWidth(e.target.value)}
          min={1}
          max={30}
...
const UndoRedo = ({ undo, redo, undoButton, redoButton}) => {
  return (
    <div className="Control">
      <h3>Undo / Redo:</h3>
      <div className="UndoRedoButtonsContainer">
        <button onClick={() => undo()}  disabled={undoButton}
          style={{
            cursor: undoButton ? "not-allowed" : null,
          }}>

          <ImUndo className="ButtonIcon" />
          Undo
        </button>{" "}
        <button onClick={() => redo()} disabled={redoButton}
          style={{
            cursor: undoButton ? "not-allowed" : null,
          }} >
  ...
  return (
    <div className="ControlPanel">
      ...
      <BorderWidth
        ...
        getFinalBorderWidth = {getFinalBorderWidth}
        getBorderWidthBeforeMove = {getBorderWidthBeforeMove}
      />
      <FillColor
        currFillColor={currFillColor}
      ...
      />
      <UndoRedo undo={undo} redo={redo} undoButton={undoButton}  redoButton={redoButton}/>
    </div>
  );
};
